...
const Page = () => {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^\d{4}$/.test(formData.yearOfManufacture)
    ) {
    ...
    if (formData.pricePerMonth && isNaN(formData.pricePerMonth)) {
      newErrors.pricePerMonth =
        "Please enter a valid number for price per month";
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const userConfirmed = window.confirm(
      "Are you sure you want to upload the car details?"
    );
